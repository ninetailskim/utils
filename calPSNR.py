...
if __name__ == '__main__':
    ...
    for root, dirs, files in list_files:
        for d in dirs:
            ...
            index = 0
            idx = [2,5,8,11,15,18,21,24,28,31,34,38,41,44,47,51,54,57,60,64,67,70,74,77,80,83,87,90,93,97]
            images = []
            for image in tempimages:
                if int(image.split('\\')[-1].split('.')[0]) in idx:
                    images.append(image)
            print(len(images))
            for image in images:
                
                GTpath = os.path.join('GT', d,'GT', image.split('\\')[-1].split('.')[0]+'.jpg')
                Bpath = os.path.join('GT', d,'input', image.split('\\')[-1].split('.')[0]+'.jpg')
                print(image)
                print(GTpath)
                #print(Bpath)
            ...
            fo.write(d+'average:  psnr:' +str(senceGTPSNR/index)+'/'+str(sencePSNR/index) + '   ssim:' + str(senceGTSSIM/index)+'/'+str(senceSSIM/index)+ '\n')
            fsence.flush()
            fo.flush()
            fsence.close()
